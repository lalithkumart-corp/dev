...
window.webcam = {
	...
	/*upload: function(url, callback) {debugger;
		...
		this.get_movie().upload( this.api_url);
	},*/
...
gs.webcamModel = {
	...
	init: function(){
		//give the php file path
			...
			var camera = $('#camera');
			camera.html(webcam.get_html(350, 350)); //generate and put the flash embed code on page
			...
			webcam.set_hook( 'onComplete', function(img){
				...
				$('#show_saved_img').html('<img src="' + tt[0] + '">');
				gs.webcamModel.displayPicName = tt[1];
				$('#savePhoto').fadeIn();
				$('#deletePhoto').fadeIn();
